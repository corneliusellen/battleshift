...
describe "Api::V1::Games" do
  context "POST /api/v1/games/:id/shots" do
    ...
    it "player 2 fires shot and misses" do
      headers = { "X-API-KEY" => @user_1.api_key.api_key,
        "CONTENT-TYPE" => "application/json" }
        params_shot = {target: "A1"}.to_json
        post "/api/v1/games/#{Game.last.id}/shots", params: params_shot, :headers => headers

      headers_2 = { "X-API-KEY" => @user_2.api_key.api_key,
                  "CONTENT-TYPE" => "application/json" }
      params_shot_2 = {target: "D1"}.to_json
      post "/api/v1/games/#{Game.last.id}/shots", params: params_shot_2, :headers => headers_2

      expect(response.status).to eq(200)

      result = JSON.parse(response.body)

      status_space = result["player_1_board"]["rows"][3]["data"].first["status"]
      expect(status_space).to eq("Miss")
      expect(result["message"]).to include("Your shot resulted in a Miss")
    end

    it "Player 2 attempts to fire again but it isn't their turn" do
      headers = { "X-API-KEY" => @user_2.api_key.api_key,
                  "CONTENT-TYPE" => "application/json" }
